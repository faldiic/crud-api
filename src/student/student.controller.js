...
const { getAllStudents, getStudentById, createStudent, deleteStudentById, editStudentById } = require('./student.service');
...
router.get("/", async (req, res) => {
    try {
        ...
        res.status(500).send({ message: "Failed to fetch students", error: err.message }); // 500 Internal Server Error
    }
});
...
router.post("/", async (req, res) => {
    try {
        ...
        res.status(400).send({ message: "Failed to create student", error: err.message }); // 400 Bad Request
    }
});
...
router.put("/:npm", async (req, res) => {
    try {
        ...
        res.status(500).send({ message: "Failed to update student", error: err.message }); // 500 Internal Server Error
    }
});
